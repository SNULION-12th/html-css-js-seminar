...
storyModal.addEventListener("click", (event) => {
  ...
  if (target == event.currentTarget.querySelector("#story-image")) {
    return;
  }
...
// 과제 1) 로고 클릭 시 페이지 새로고침
const headerlogo = document.querySelector(".logo");
headerlogo.addEventListener("click", () => {
  location.reload();
});

// 과제 2) 아이콘 hover 시 불투명하게
...
const articlearea = document.querySelector(".article-container");
articlearea.addEventListener("mouseover", () => {
  articlearea.style.opacity = "0.7";
});
articlearea.addEventListener("mouseout", () => {
  articlearea.style.opacity = "1";
});

// 과제 3) 좋아요 버튼 클릭 시 알림창 띄우기
...
emoji_buttons.forEach((emoji_button) => {
  emoji_button.addEventListener("click", (event) => {
    document.querySelector(".comment").value += event.target.textContent;
  });
});

// emoji_buttons.forEach((emoji_button) => {
//   emoji_button.addEventListener("click", (event) => {
//     var txtArea = document.querySelector(".comments-create-form");
...
//     var addTxt = emoji_button.InnerText; // 추가 입력 할 텍스트
